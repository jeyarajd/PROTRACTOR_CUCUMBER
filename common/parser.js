...
module.exports = function Parser(builder) {
  ...
  this.parse = function(tokenScanner, tokenMatcher) {
    ...
    while(true) {
      ...
      if (token.line != null && token.line.lineText != null && token.line.lineText.includes(".csv")) {
          var lineCount = 1;
          var data = fs.readFileSync(token.line.lineText, 'utf8');
          var dataArray = data.split(/\r?\n/);  
          var lineNum = token.location.line;
          var columnNum = token.location.column;
          var items = [];
          dataArray.forEach((entry) => {
              var row = entry.trim();
              var location = {};
              location.line = lineNum;
              location.column == columnNum;
              var tokn = new Token(new GherkinLine(row, lineNum), location)
              state = matchToken(state, tokn, context);
              lineNum = lineNum + 1;
          })
      }
      else {
